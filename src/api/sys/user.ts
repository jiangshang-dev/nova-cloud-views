...
import { USE_NOVA_AUTH, novaGetCaptcha, novaGetUserInfoApi, novaLoginApi, novaLogoutApi } from '/@/api/nova/auth';
 ...
 */
export function loginApi(params: LoginParams & Recordable, mode: ErrorMessageMode = 'modal') {
  if (USE_NOVA_AUTH) {
    return novaLoginApi(
      {
        ...
        checkKey: String(params.checkKey ?? ''),
      },
      mode,
...
export function getUserInfo() {
  const request = USE_NOVA_AUTH
    ? novaGetUserInfoApi()
    : defHttp.get<GetUserInfoModel>({ url: Api.GetUserInfo }, {});
...
export function getCodeInfo(currdatetime) {
  if (USE_NOVA_AUTH) {
    return novaGetCaptcha(currdatetime);
  }
