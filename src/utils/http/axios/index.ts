...
const transform: AxiosTransform = {
  ...
  transformRequestHook: (res: AxiosResponse<Result>, options: RequestOptions) => {
    ...
    // Nova 约定：code + data/message；兼容 Jeecg：code + result/message
    const { code, result, data: payload, message, msg, success } = data as any;
    const tipMessage = message ?? msg;
    const bizData = payload !== undefined ? payload : result;
    // 成功码：200（Nova）/ 0（Jeecg 兼容）
    const hasSuccess =
      data &&
      Reflect.has(data, 'code') &&
      (code === ResultEnum.SUCCESS || code === ResultEnum.SUCCESS_COMPAT || code === 200);
    if (hasSuccess) {
      if (success && tipMessage && options.successMessageMode === 'success') {
        createMessage.success(tipMessage);
      }
      return bizData;
    }
...
function createAxios(opt?: Partial<CreateAxiosOptions>) {
  return new VAxios(
    deepMerge(
      {
        // Nova 网关/认证默认 Bearer；同时 requestInterceptors 仍会写 X-Access-Token 兼容头
